    ...
    useEffect(() => {
        lanesRef.current.map( (lane, i) => {
            lane.map( (img, j) => {
                ...
                img.tl = gsap.timeline({paused:true, repeat:-1})
                .fromTo(img, {
                        y:[-575,500,500][i], 
                        rotation:-0.05
                    }, {
        ...
        gsap.timeline({onStart:playBoxes,
            ...
            .set(mainBoxRef.current,    {left:'75%', xPercent:-5, width:1200, rotationX:14, rotationY:-15, rotationZ:10})
            .set(mainCloseRef.current,  {autoAlpha:0, width:60, height:60, left:-30, top:-31, pointerEvents:'none'})
            .fromTo(wrapperRef.current, {autoAlpha:0},{duration:2, ease:'power2.inOut', autoAlpha:1}, 0.2)
